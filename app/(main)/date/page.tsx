...
import { 
  MapPin, ShoppingBag, Coffee, TreePine, Book, Gamepad2, Soup, 
  UtensilsCrossed, Landmark, Ticket, Camera, Music, Waves, Lock, 
  Sparkles, Film, Fish, Snowflake, Send, LucideIcon 
} from 'lucide-react';
...
const LOCATIONS: LocationDef[] = [
  { id: 'supermarket', name: 'Supermarket', icon: ShoppingBag, color: 'bg-blue-50 border-blue-200 text-blue-600', hover: 'hover:bg-blue-100' },
  { id: 'taman', name: 'Taman Kota', icon: TreePine, color: 'bg-green-50 border-green-200 text-green-600', hover: 'hover:bg-green-100' },
  { id: 'cafe', name: 'Kafe Kucing', icon: Coffee, color: 'bg-amber-50 border-amber-200 text-amber-600', hover: 'hover:bg-amber-100' },
  { id: 'perpustakaan', name: 'Perpustakaan', icon: Book, color: 'bg-purple-50 border-purple-200 text-purple-600', hover: 'hover:bg-purple-100' },
  { id: 'arcade', name: 'Arcade Center', icon: Gamepad2, color: 'bg-indigo-50 border-indigo-200 text-indigo-600', hover: 'hover:bg-indigo-100' },
  { id: 'ramen', name: 'Warung Ramen', icon: Soup, color: 'bg-orange-50 border-orange-200 text-orange-600', hover: 'hover:bg-orange-100' },
  { id: 'gyoza', name: 'Restoran Gyoza', icon: UtensilsCrossed, color: 'bg-red-50 border-red-200 text-red-600', hover: 'hover:bg-red-100' },
  { id: 'museum', name: 'Museum Seni', icon: Landmark, color: 'bg-stone-50 border-stone-200 text-stone-600', hover: 'hover:bg-stone-100' },
  { id: 'amusement', name: 'Taman Hiburan', icon: Ticket, color: 'bg-pink-50 border-pink-200 text-pink-600', hover: 'hover:bg-pink-100' },
  { id: 'studio', name: 'Studio Potret', icon: Camera, color: 'bg-teal-50 border-teal-200 text-teal-600', hover: 'hover:bg-teal-100' },
  { id: 'konser', name: 'Konser Musik', icon: Music, color: 'bg-indigo-50 border-indigo-200 text-indigo-600', hover: 'hover:bg-indigo-100', requiredItem: 'tiket_konser', requirementName: 'Tiket Konser' },
  { id: 'festival', name: 'Festival Musim Panas', icon: Sparkles, color: 'bg-rose-50 border-rose-200 text-rose-600', hover: 'hover:bg-rose-100', requiredItem: 'outfit_yukata', requirementName: 'Yukata Festival', mustWear: true },
  { id: 'pantai', name: 'Pantai', icon: Waves, color: 'bg-cyan-50 border-cyan-200 text-cyan-600', hover: 'hover:bg-cyan-100' },
  { id: 'bioskop', name: 'Bioskop', icon: Film, color: 'bg-slate-50 border-slate-200 text-slate-600', hover: 'hover:bg-slate-100' },
  { id: 'akuarium', name: 'Akuarium', icon: Fish, color: 'bg-blue-50 border-blue-200 text-blue-600', hover: 'hover:bg-blue-100' },
  { id: 'food_court', name: 'Food Court', icon: UtensilsCrossed, color: 'bg-amber-50 border-amber-200 text-amber-600', hover: 'hover:bg-amber-100' },
  { id: 'ice_skating', name: 'Ice Skating', icon: Snowflake, color: 'bg-sky-50 border-sky-200 text-sky-600', hover: 'hover:bg-sky-100' },
];

const LOC_BG_MAP: Record<string, { folder: string; prefix: string }> = {
...
function getDateBackgroundUrl(locName: string | null, timeOfDay: 'pagi' | 'sore' | 'malam'): string {
  if (!locName) return '';
  const config = LOC_BG_MAP[locName];
  return config ? `/date/${config.folder}/${config.prefix}_${timeOfDay}.jpg` : '';
}
...
function DateContent() {
  ...
  const itemsSet = useMemo(() => new Set(itemsBrought), [itemsBrought]);

  // Initial load: fetch outfit & inventory once
  useEffect(() => {
    fetch('/api/outfit')
  ...
  // Preload expressions to eliminate image flickering during date
  useEffect(() => {
    const outfitFolder = activeOutfit || 'default';
    ['normal', 'happy', 'blushing', 'angry', 'clingy'].forEach(expr => {
      const img = new Image();
      img.src = `/livia/home-screen/${outfitFolder}/${expr}.webp`;
    });
  }, [activeOutfit]);

  // Handle URL query parameter ?location=...
  useEffect(() => {
    const locationParam = searchParams.get('location');
    if (locationParam && !didAutoStart.current) {
  ...
  const startJalan = async (locName: string) => {
    ...
    // Preload morning background image as early as possible
    const bgUrl = getDateBackgroundUrl(locName, 'pagi');
    if (bgUrl) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data?.scene && Array.isArray(data.scene) && data.scene.length > 0) {
          setScene(data.scene);
  ...
  const handleNextScene = () => {
    ...
    if (scene && sceneIndex < scene.length - 1) {
      ...
      setLiviaExpression(scene[nextIdx].expression || 'normal');
    } else {
  ...
  const lastChat = chatLog.length > 0 ? chatLog[chatLog.length - 1] : null;
  const dialogText = isInteractive
    ? (isTyping ? '...' : (lastChat?.content || ''))
    : (scene && scene[sceneIndex] ? scene[sceneIndex].text : '');

  const dialogSpeaker = isInteractive
    ? (isTyping ? 'Livia' : (lastChat?.role === 'livia' ? 'Livia' : lastChat?.role === 'user' ? 'Kamu' : ''))
    : (scene && scene[sceneIndex] ? (scene[sceneIndex].speaker === 'Narator' ? '' : scene[sceneIndex].speaker) : '');
  ...
  return (
    <div className="h-screen max-h-screen bg-[#fdfbf7] relative overflow-hidden flex flex-col font-sans select-none">
      ...
      {scene ? (
        ...
        <div className="fixed inset-0 z-[100] bg-[#fdfbf7] flex flex-col items-center justify-between py-6 px-4 md:px-6 sm:py-12 animate-[fadeIn_0.3s_ease-out]">
          ...
          <div className="w-full max-w-5xl flex justify-between px-2 sm:px-8 z-20 shrink-0 mt-8 md:mt-0">
            ...
            <button 
              ...
              className="text-gray-400 hover:text-[#ff758c] text-sm md:text-base font-bold bg-white px-4 md:px-6 py-1.5 md:py-2 rounded-full shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all"
            >
            ...
            <div className={`w-full transition-all duration-500 transform ${isInteractive ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0 pointer-events-none'}`}>
              <div className="px-2 md:px-0 flex items-end gap-2 md:gap-3">
                ...
                <button
                  ...
                  className="shrink-0 w-12 h-12 md:w-14 md:h-14 flex items-center justify-center rounded-full transition-all shadow-md bg-gradient-to-r from-[#ff0844] to-[#ffb199] text-white disabled:opacity-50 disabled:grayscale hover:shadow-lg active:scale-95"
                >
        ...
        // Location Selection Mode (Carousel)
        <div className="flex-1 max-w-[1400px] w-full mx-auto flex flex-col justify-center pt-12 md:pt-14 px-4 md:px-8 z-10 relative overflow-hidden">
          ...
          {isLoading ? (
            ...
            <div className="flex gap-4 md:gap-6 pb-4 md:pb-6 overflow-x-auto overflow-y-hidden snap-x snap-mandatory hide-scrollbar pr-4 md:pr-8 -mx-4 md:-mx-8 px-6 md:px-12 items-center">
              {LOCATIONS.map(loc => {
                const notOwned = loc.requiredItem ? !itemsSet.has(loc.requiredItem) : false;
                const notWearing = loc.mustWear && loc.requiredItem ? (activeOutfit !== loc.requiredItem && activeOutfit !== loc.requiredItem.replace('outfit_', '')) : false;
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`absolute -right-6 -top-6 opacity-10 transition-all duration-700 z-0 pointer-events-none ${
                      isLocked ? 'text-gray-400' : `${loc.color.split(' ')[2]} group-hover:scale-110 group-hover:rotate-12 group-hover:opacity-20`
                    }`}>
                    ...
                    <div className={`relative z-10 transition-transform duration-500 ease-out flex flex-col h-full justify-between ${isLocked ? '' : 'translate-y-3 md:translate-y-4 group-hover:translate-y-0'}`}>
                      ...
                      <div className="flex justify-between items-start w-full">
                        <div className={`w-12 h-12 md:w-14 md:h-14 rounded-[1.2rem] md:rounded-2xl flex items-center justify-center shadow-sm border transition-all duration-500 ${
                          ...
                          {isLocked ? <Lock size={20} className="w-5 h-5 md:w-6 md:h-6" /> : <Icon size={28} />}
                        </div>
